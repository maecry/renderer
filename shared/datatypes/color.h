...
typedef std::uint32_t ColorPacked_t;

#pragma region color_definitions
#if defined(DRAW_USE_BGRA_PACKED_COLOR)
#define DRAW_COL32_R_SHIFT 16
#define DRAW_COL32_G_SHIFT 8
#define DRAW_COL32_B_SHIFT 0
#else
#define DRAW_COL32_R_SHIFT 0
#define DRAW_COL32_G_SHIFT 8
#define DRAW_COL32_B_SHIFT 16
#endif

#define DRAW_COL32_A_SHIFT 24
#define DRAW_COL32_A_MASK 0xFF000000

#define DRAW_COL32(R, G, B, A) (((ColorPacked_t)(A) << DRAW_COL32_A_SHIFT) | ((ColorPacked_t)(B) << DRAW_COL32_B_SHIFT) | ((ColorPacked_t)(G) << DRAW_COL32_G_SHIFT) | ((ColorPacked_t)(R) << DRAW_COL32_R_SHIFT))
#define DRAW_COL32_WHITE DRAW_COL32(255, 255, 255, 255) // 0xFFFFFFFF
#define DRAW_COL32_BLACK DRAW_COL32(0, 0, 0, 255) // 0XFF000000
#define DRAW_COL32_BLACK_TRANS DRAW_COL32(0, 0, 0, 0) // 0x00000000
#pragma endregion
...
struct Color_t
{
	...
	explicit constexpr Color_t( const std::uint32_t uPackedColor ) :
		r( static_cast< std::uint8_t >( ( uPackedColor >> DRAW_COL32_R_SHIFT ) & 0xFF ) ), g( static_cast< std::uint8_t >( ( uPackedColor >> DRAW_COL32_G_SHIFT ) & 0xFF ) ), b( static_cast< std::uint8_t >( ( uPackedColor >> DRAW_COL32_B_SHIFT ) & 0xFF ) ), a( static_cast< std::uint8_t >( ( uPackedColor >> DRAW_COL32_A_SHIFT ) & 0xFF ) )
	{ }
	...
	[[nodiscard]] constexpr std::uint32_t GetU32( const float flAlphaMultiplier = 1.0f ) const
	{
		return DRAW_COL32( r, g, b, a * flAlphaMultiplier );
	}

	std::uint8_t& operator[]( const std::uint8_t nIndex )
	{
		CRT_ASSERTION( nIndex >= COLOR_R && nIndex <= COLOR_A );
	...
	Color_t Blend( const Color_t& colEnd, float flRate )
	{
		const std::uint8_t nDiffRed = colEnd.r - r;
		const std::uint8_t nDiffGreen = colEnd.g - g;
		const std::uint8_t nDiffBlue = colEnd.b - b;
		const std::uint8_t nDiffAlpha = colEnd.a - a;

		return {
			static_cast< std::uint8_t >( CRT::Clamp( r + nDiffRed * flRate, 0.0f, 255.0f ) ),
			static_cast< std::uint8_t >( CRT::Clamp( g + nDiffGreen * flRate, 0.0f, 255.0f ) ),
			static_cast< std::uint8_t >( CRT::Clamp( b + nDiffBlue * flRate, 0.0f, 255.0f ) ),
			static_cast< std::uint8_t >( CRT::Clamp( a + nDiffAlpha * flRate, 0.0f, 255.0f ) )
		};
	}
